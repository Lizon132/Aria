...
def calculate_score(flight, price_weight=0.5, duration_weight=0.3, stops_weight=0.2):
...
def find_top_flights(flights, num_flights):
    sorted_flights = sorted(flights, key=lambda x: calculate_score(x))
...
if __name__ == "__main__":
    ...
    top_flights = find_top_flights(flights, num_results)
    flights_dict = dict()
    for flight in top_flights:
        flights_dict[flight.flight_id] = flight
